    ...
    internal static class Program
    {
        ...
        private static int Main(string[] args)
        {
            ...
            client.CommandReceived += command =>
            {
                ...
                else if (command.StartsWith("DUMP", StringComparison.OrdinalIgnoreCase))
                {
                    DataReaders? activeReader = reader;
                    if (activeReader == null)
                    {
                        client.SendStatus("dump_unavailable", "WRAM is not attached yet.");
                        return;
                    }

                    try
                    {
                        byte[] wram = activeReader.ReadWramRange(0, Lufia2MemoryMap.Wram.Size);
                        string hex = Convert.ToHexString(wram);
                        client.SendRaw(
                            $"{{\"type\":\"dump\",\"size\":{wram.Length},\"data\":\"{hex}\"}}");
                    }
                    catch (Exception ex)
                    {
                        client.SendStatus("dump_error", $"WRAM dump failed: {ex.Message}");
                    }
                }
            };
            ...
            while (true)
            {
                try
                {
                    ...
                    if (process != null && reader != null)
                    {
                        ...
                        {
                            // The AI player consumes one merged state document.
                            // Send a complete snapshot so newly added live fields
                            // are never reset by default values from a partial DTO.
                            client.SendState(state);
                            lastState = state;
                        }
                    }
        ...
        private static bool CoreStateEquals(GameState current, GameState? previous)
        {
            ...
            return SequenceEqual(current.Inventory, previous.Inventory) &&
                   ...
                   SequenceEqual(current.PartyStats, previous.PartyStats) &&
                   SequenceEqual(current.RawInventory, previous.RawInventory) &&
                   SequenceEqual(current.EnemyHpList, previous.EnemyHpList) &&
                   SequenceEqual(current.EnemyStats, previous.EnemyStats) &&
                   current.Gold == previous.Gold &&
                   current.MapId == previous.MapId &&
                   current.PreviousMapId == previous.PreviousMapId &&
                   current.DungeonX == previous.DungeonX &&
                   current.DungeonY == previous.DungeonY &&
                   current.DungeonBlocking == previous.DungeonBlocking &&
                   current.MapContextFlag == previous.MapContextFlag &&
                   current.ExplorationModeFlag == previous.ExplorationModeFlag &&
                   current.BattleModeFlag == previous.BattleModeFlag &&
                   current.FacingContactState == previous.FacingContactState &&
                   current.DungeonAxisMirrorX == previous.DungeonAxisMirrorX &&
                   current.DungeonAxisMirrorY == previous.DungeonAxisMirrorY &&
                   current.DungeonAxisChunkY == previous.DungeonAxisChunkY &&
                   current.InBattle == previous.InBattle &&
